...
[모던 자바스크립트 Deep Dive](http://www.yes24.com/Product/Goods/92742567) 책을 참고하고 쓴 것임을 미리 알려드립니다!

<div className="w-52">![blackout](/static/images/blackout.jpeg)</div>

<br /> 혹시나 면접 전에 머리가 하얘져서 기억이 안난다면? 아래 글을 빠르게 습득합시다! 고고
...
`ES6`에서 등장한 `let`, `const`의 유효 범위는 모두 블록 스코프입니다. 함수 스코프로 동작하던 `var` 키워드의 예측하지 못한 동작을 방지할 수 있어서, Modern JavaScript에서는 `let`, `const`를 사용하는 것이 권장됩니다.
...
이번 글에서는 var, let, const 키워드에 대해 알아봤습니다. JavaScript의 변수의 동작원리도 보면 재미있는게 많은데, 이번 글의 주제와는 벗어나니까 다음 글에서 작성하도록 하겠습니다. 감사합니다~!
