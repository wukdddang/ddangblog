...
이렇게 만들어진 **ER**에는 상위 렉시컬 스코프를 참조하는 OuterEnv 필드가 존재합니다. 

그러니까 `블록 스코프와 함수 스코프에서 실행 컨텍스트가 생성되는 것`입니다. 저는 이부분이 조금 헷갈렸는데, ES5 이전의 JavaScript는 함수 스코프의 렉시컬 스코프만 있었다면, 
`ES6 이후의 JavaScript는 블록 스코프도 렉시컬 스코프로서 동작할 수 있게 되었다`는 의미입니다. 와우!
...
> 자바스크립트 코드가 실행될 때 필요한 정보들을 가지고 있는 맥락! 
...
- **LE**는 런타임에 변경될 수 있지만(예: 블록 내에서 let으로 선언된 변수), **VE**는 코드 실행 전에 설정되며 런타임에 변경되지 않습니다.
...
hello 실행 컨텍스트를 살펴볼까요? 오, 함수 호출만 2개가 있네요. 이 경우 식별자 저장 없이 JS 엔진은 그저 실행 컨텍스트를 순차적으로 실행합니다. 
...
let, const는 TDZ(Temporal Dead Zone)가 존재해서 개발자의 실수를 방지해주는 반면에, var은 TDZ가 없습니다. 자세한 건 여기서 봐주세요 😆 ([var, let, const의 차이점에 대해 설명해주세요](/blog/interviews/var-let-const))
...
`1`일까요 `3`일까요? 네 당연히 `1`입니다. hello 실행 컨텍스트의 OuterEnv는 전역 실행 컨텍스트이기 때문에, 
