...
`환경 레코드`라고도 하는 `Environment Records`는 (이하 ER) 현재 실행 컨텍스트와 관련된 코드의 식별자 정보들이 저장됩니다. (아직 실행 컨텍스트가 무엇인지 잘 모르시는 분들은, 아래 글을 보고 다시 이 내용을 이해하시면 좋을 것 같습니다.) 
...
그럼, 이 때 첫 번째 줄의 `hi`는 전역 실행 컨텍스트의 ER에 저장이 되겠죠? 